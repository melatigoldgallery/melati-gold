  ...
  <div>
    ...
    <div class="lg:hidden mb-4">
      <button
        ...
        class="w-full px-4 py-3 bg-white border border-gray-300 rounded-lg flex items-center justify-between hover:bg-gray-50 transition-colors"
      >
        <span class="flex items-center gap-2">
          <i class="bi bi-funnel"></i>
          <span class="font-semibold">Filter & Urutkan</span>
        </span>
        <span v-if="hasActiveFilters" class="text-xs bg-maroon text-white px-2 py-1 rounded-full">Aktif</span>
      </button>
    </div>
    ...
    <div class="hidden lg:block bg-white rounded-xl shadow-sm p-6 sticky top-4">
      <div class="flex items-center justify-between mb-6">
        ...
        <button v-if="hasActiveFilters" @click="clearFilters" class="text-sm text-maroon hover:underline">Reset</button>
      </div>

      <!-- Subcategories -->
      <div class="mb-6 border-b pb-6">
        <h4 class="text-sm font-semibold text-gray-700 mb-3">Sub-Kategori</h4>
      ...
      <!-- Price Range -->
      <div class="mb-6 border-b pb-6">
        <h4 class="text-sm font-semibold text-gray-700 mb-3">Harga</h4>
        <div class="space-y-2">
          <button
            ...
            :class="
              isPriceRangeActive(range.min, range.max) ? 'bg-maroon text-white' : 'text-gray-700 hover:bg-gray-100'
            "
          >
            {{ range.label }}
          </button>
      ...
      <!-- Karat -->
      <div class="mb-6 border-b pb-6">
        <h4 class="text-sm font-semibold text-gray-700 mb-3">Karat</h4>
        <div class="space-y-2">
          <label
      ...
      <!-- Sort -->
      <div>
        <h4 class="text-sm font-semibold text-gray-700 mb-3">Urutkan</h4>
        <select
          v-model="localFilters.sortBy"
          @change="updateFilters"
          class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-maroon focus:border-maroon"
        >
        ...
        <div v-if="showMobileFilters" class="fixed inset-0 z-50 lg:hidden" @click.self="showMobileFilters = false">
          ...
          <div class="absolute bottom-0 left-0 right-0 bg-white rounded-t-2xl max-h-[85vh] overflow-y-auto">
            <div class="sticky top-0 bg-white border-b px-4 py-4 flex items-center justify-between">
              <h3 class="text-lg font-semibold">Filter & Urutkan</h3>
              <button @click="showMobileFilters = false" class="w-8 h-8 flex items-center justify-center">
            ...
            <div class="p-4">
              ...
              <div class="flex gap-3 pt-4 border-t">
                <button
                  ...
                  @click="clearFilters"
                  class="flex-1 px-4 py-3 border border-gray-300 rounded-lg font-semibold hover:bg-gray-50"
                >
                  Reset
                </button>
                <button
                  @click="showMobileFilters = false"
                  class="flex-1 px-4 py-3 bg-maroon text-white rounded-lg font-semibold hover:bg-maroon-dark"
                >
                  Tampilkan Hasil
                </button>
              </div>
