  ...
  <section id="best-produk" class="container py-10 md:py-14">
    <div class="mx-auto mb-6 text-center max-w-2xl reveal-in">
      ...
      <p class="mt-2 text-neutral-600">
        Koleksi favorit pilihan pelanggan, siap memancarkan pesonamu.
      </p>
    </div>
    ...
    <div v-if="loading" class="text-center py-12">
      <div
        class="inline-block animate-spin rounded-full h-12 w-12 border-b-2 border-yellow-600"
      ></div>
    ...
    <div v-else class="grid gap-7 grid-cols-2 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4">
      ...
      >
        <div class="relative">
          ...
          <span v-if="p.is_best_seller" class="chip absolute right-3 top-3 bg-yellow-600 text-white"
            >Best</span
          >
        </div>
        <div class="px-3 py-2 flex flex-col gap-2">
          ...
          <div class="flex items-center justify-between gap-1 md:gap-2 min-w-0">
            <h3
              class="font-medium text-maroon flex-1 min-w-0 truncate leading-tight text-sm md:text-base"
            >
              {{ p.title || p.name }}
            </h3>
          </div>
          ...
          <p class="text-sm text-neutral-600">
            {{ p.karat ? p.karat + ' • ' : '' }}{{ formatPrice(p.price) }}
          </p>
