...
// Regular HTTP function untuk keep-alive Supabase database
// Akan dipanggil oleh external cron service setiap hari
export const handler: Handler = async (event: HandlerEvent, context: HandlerContext) => {
  try {
    ...
    const { data, error } = await supabase.from("catalog_products").select("id").limit(1);
